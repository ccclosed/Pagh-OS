...
- **TLS**: аутентификация сервера включена fail-closed (цепочка → CA-бандл, SAN-hostname, validity + clock gate, `CertificateVerify`); отказ прерывает handshake, ветка «нет верификатора ⇒ пропустить проверку» недостижима. Доверие ограничено **четырьмя пиннутыми корнями** (`ca_bundle.rs`: ISRG Root X1/X2, GTS R1/R4), поэтому HTTPS-зеркало вне этой выдачи отвергается (`ChainError::NoAnchor`) — лечится только перегенерацией бандла `tools/gen_ca_bundle.py`; якоря парсятся лениво один раз за загрузку (кэш `ANCHORS`), пустой набор логируется `warn!`. Не проверяются: отзыв сертификатов (CRL/OCSP), подписи метаданных Debian (OpenPGP) и полнота digest-проверки пакетов; HTTP-зеркала (`apt setmirror http://…`) неаутентифицированы по построению.
  RNG (`KernelRng` над RDSEED/RDRAND) fail-closed. Диагностика верификатора — только `error!`-строки `Package_Fetcher(tls): stage=verify cause=…` (`InvalidCertificate` / `InvalidSignatureScheme` / `InvalidSignature`): собственные сообщения embedded-tls идут на уровне `debug!`, а фасад по умолчанию фильтрует всё выше `Info`.
