...
| `ca_bundle.rs` | **Сгенерированный** (tools/gen_ca_bundle.py) trust-anchor бандл: метка + raw DER самоподписанных корней (ISRG Root X1/X2 — цепочки Let's Encrypt, которыми подписан deb.debian.org; GTS R1/R4 — CDN). Это **вся** доверенная выдача: HTTPS-зеркало под другим корнем отвергается (перегенерация бандла — осознанный акт, пины sha256 в `SELECTION`). Разбирается парсером ядра один раз за загрузку и кэшируется в `net::tls::ANCHORS`; обновлять только перегенерацией, файл коммитится; P48 доказывает валидность каждого якоря kernel-парсером и диспетчером подписей |
| `tls_auth.rs` | Однократное server-authentication-решение, подключённое к handshake: `authenticate_server(entries, host, anchors, now)` = chain verify (через `tls_chain`) + hostname-авторизация **только по SAN** (dNSName для DNS-цели, iPAddress по октетам для IP-литерала; CN-fallback отсутствует, отсутствие/битость SAN — отказ) + экспорт leaf-ключа (`LeafKey`, owned) для шага CertificateVerify. Плюс точное сообщение RFC 8446 §4.4.3 (`certificate_verify_message`) и `verify_certificate_verify(scheme, cipher_suite_hash_len, hash, sig, key)`: транскрипт-хеш гейтится шириной хеша **negotiated cipher suite** (32 для `Aes128GcmSha256`), а не длиной хеша схемы — схема задаёт только дайджест подписи, поэтому P-384/RSA-PSS-384/512 листья проверяемы (три бэкенда не мёртвые). Хост не задан → `NoHostname`; ключ вне supported surface → `UnsupportedLeafKey` |
...
- **TLS**: аутентификация сервера включена fail-closed (цепочка → CA-бандл, SAN-hostname, validity + clock gate, `CertificateVerify`); отказ прерывает handshake, ветка «нет верификатора ⇒ пропустить проверку» недостижима. Рукопожатие **без** `Certificate`/`CertificateVerify` — отдельный отказ: вендорный патч `embedded-tls` (латчи `certificate_received`/`certificate_verified`; PSK-рукопожатия исключены) не принимает server `Finished` без них — иначе пир мог дойти до `ApplicationData`, ни разу не вызвав верификатор, — а `https_get` независимо требует, чтобы `VERIFIED_HANDSHAKES` сдвинулся за время handshake. Доверие ограничено **четырьмя пиннутыми корнями** (`ca_bundle.rs`: ISRG Root X1/X2, GTS R1/R4), поэтому HTTPS-зеркало вне этой выдачи отвергается (`ChainError::NoAnchor`) — лечится только перегенерацией бандла `tools/gen_ca_bundle.py`; якоря парсятся лениво один раз за загрузку (кэш `ANCHORS`), пустой набор логируется `warn!`. Не проверяются: отзыв сертификатов (CRL/OCSP), подписи метаданных Debian (OpenPGP) и полнота digest-проверки пакетов; HTTP-зеркала (`apt setmirror http://…`) неаутентифицированы по построению.
  RNG (`KernelRng` над RDSEED/RDRAND) fail-closed. Диагностика верификатора — только `error!`-строки `Package_Fetcher(tls): stage=verify cause=…` (`InvalidCertificate` / `InvalidSignatureScheme` / `InvalidSignature`): собственные сообщения embedded-tls идут на уровне `debug!`, а фасад по умолчанию фильтрует всё выше `Info`. Отдельный `cause=InvalidCertificate` печатает сам `https_get`, когда handshake завершился, но счётчик аутентификаций не сдвинулся.
