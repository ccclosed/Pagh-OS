...
pub fn sys_getpid() -> Result<u64, Errno> { Ok(compat::current_tgid()) }
...
pub fn sys_set_tid_address(tidptr: u64) -> Result<u64, Errno> {
    if tidptr != 0 { check_user_ptr(tidptr, 4)?; }
    let tid = compat::with_current_compat(|cs| { cs.clear_child_tid=tidptr; cs.tid }).unwrap_or_else(scheduler::current_pid);
    Ok(tid)
}
...
pub fn sys_exit_group(code:u64)->!{
    let byte=exit_code_byte(code); let current=scheduler::current_pid(); let tgid=compat::current_tgid();
    let others=compat::group_member_pids(tgid,current);
    for pid in others.iter().copied(){ super::process_sys::cleanup_thread_exit(pid); compat::remove_compat(pid); }
    scheduler::remove_ready_pids(&others); compat::with_current_compat(|cs|cs.exit_code=Some(byte));
    super::process_sys::cleanup_current_thread_exit();
...
pub fn sys_getppid() -> Result<u64, Errno> { Ok(compat::current_ppid()) }
...
pub fn sys_set_robust_list(head: u64, len: u64) -> Result<u64, Errno> {
    ...
    if len != ROBUST_HEAD_SIZE { return Err(Errno::EINVAL); }
    check_user_ptr(head,len)?;
    compat::with_current_compat(|cs| { cs.robust_head=head; cs.robust_len=len; }).ok_or(Errno::EINVAL)?;
    Ok(0)
}
pub fn sys_get_robust_list(pid:u64,head_ptr:u64,len_ptr:u64)->Result<u64,Errno>{
    let current=scheduler::current_pid(); if pid!=0 && pid!=current{return Err(Errno::ESRCH)}
    check_user_ptr(head_ptr,8)?;check_user_ptr(len_ptr,8)?;let (head,len)=compat::current_robust_list();
    unsafe{core::ptr::write_unaligned(head_ptr as *mut u64,head);core::ptr::write_unaligned(len_ptr as *mut u64,len)} Ok(0)
}
...
/// STAGE-16.14 `setpgid` (109): accepted as a no-op. There is still no real
...
/// STAGE-16.14 `getpgrp` (111) / `getpgid` (121): every process is modeled as
