...
extern "x86-interrupt" fn invalid_tss_handler(stack: InterruptStackFrame, error_code: u64) {
    crate::error!("[EXC #10] Invalid TSS err=0x{:x}", error_code);
    halt();
}
extern "x86-interrupt" fn segment_not_present_handler(stack: InterruptStackFrame, error_code: u64) {
    crate::error!("[EXC #11] Segment NP err=0x{:x}", error_code);
    halt();
}
extern "x86-interrupt" fn stack_segment_handler(stack: InterruptStackFrame, error_code: u64) {
...
extern "x86-interrupt" fn alignment_check_handler(stack: InterruptStackFrame, error_code: u64) {
    crate::error!("[EXC #17] Alignment err=0x{:x}", error_code);
    halt();
}
extern "x86-interrupt" fn machine_check_handler(stack: InterruptStackFrame) -> ! {
...
extern "x86-interrupt" fn vmm_comm_handler(stack: InterruptStackFrame, error_code: u64) {
    crate::error!("[EXC #29] VMM Comm err=0x{:x}", error_code);
    halt();
}
extern "x86-interrupt" fn security_handler(stack: InterruptStackFrame, error_code: u64) {
