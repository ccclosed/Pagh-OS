...
/// `rust <path> [args...]`: run a statically linked Rust Linux binary.
/// This is a friendly alias for the Linux compatibility loader.
pub(super) fn cmd_rust(ctx: &mut ShellCtx, args: &[&str]) {
    if args.is_empty() {
        shell_println("usage: rust <file.rs|file.pbc|static-elf> [args...]");
        return;
    }
    if args[0].ends_with(".rs") || args[0].ends_with(".pbc") {
        if let Err(e) = super::toolchain::run_path(args[0]) {
            super::render::error_line(&alloc::format!("rust: {}", e));
        }
    } else {
        cmd_lxrun(ctx, args);
    }
...
/// Embedded offline mini-Rust package manager.
pub(super) fn cmd_cargo(ctx: &mut ShellCtx, args: &[&str]) {
    if crate::vfs::lookup_path("/mnt/usr/bin/cargo").is_ok() {
        runtime_command(ctx, "/mnt/usr/bin/cargo", &[], args);
    } else {
        super::toolchain::cargo(args);
    }
}

/// Compile mini-Rust source into a portable `.pbc` source package.
pub(super) fn cmd_rustc(ctx: &mut ShellCtx, args: &[&str]) {
    if crate::vfs::lookup_path("/mnt/usr/bin/rustc").is_ok() {
        runtime_command(ctx, "/mnt/usr/bin/rustc", &[], args);
    } else {
        super::toolchain::rustc(args);
    }
}

/// Inspect the embedded mini-Rust toolchain.
pub(super) fn cmd_rustup(_ctx: &mut ShellCtx, args: &[&str]) {
    super::toolchain::rustup(args);
}
...
pub(super) fn cmd_lua(ctx: &mut ShellCtx, args: &[&str]) {
...
fn find_python() -> Option<alloc::string::String> {
    let dir = crate::vfs::lookup_path("/mnt/usr/bin").ok()?;
    let entries = dir.readdir().ok()?;
    let mut best: Option<alloc::string::String> = None;
    for e in entries {
        let n = e.name();
        if n.starts_with("python3") && !n.ends_with(".py") {
            let full = alloc::format!("/mnt/usr/bin/{}", n);
            if best.as_ref().map_or(true, |b| full.len() > b.len()) {
                best = Some(full);
            }
        }
    }
...
pub(super) fn cmd_python(ctx: &mut ShellCtx, args: &[&str]) {
    let Some(py) = find_python() else {
        shell_println("python: no python3 in /mnt/usr/bin yet; run 'apt install python3' or wait for first-boot provisioning");
        return;
    };
...
pub(super) fn cmd_pythonc(ctx: &mut ShellCtx, args: &[&str]) {
    if args.is_empty() {
        shell_println("usage: pythonc <file.py> [file2.py ...]");
        return;
    }
    let Some(py) = find_python() else {
        shell_println("pythonc: no python3 in /mnt/usr/bin yet; run 'apt install python3' first");
        return;
    };
    let mut argv: alloc::vec::Vec<&str> = alloc::vec::Vec::with_capacity(3 + args.len());
    argv.push(py.as_str());
    argv.push("-m");
...
///
/// Transport: the normal pagh development build enables outbound package access
/// through the `network_packages` default feature. HTTPS is encrypted but still
/// unauthenticated, so every update prints a trust warning. A fail-closed kernel
/// can be built with `--no-default-features`. `setmirror` accepts a scheme prefix.
///
