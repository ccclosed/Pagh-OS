...
- **Загрузочный поток ≠ idle-задача.** До `halt_loop()` в `boot::kernel_main` текущий pid
  ...
  **не должен** случаться до конца загрузки: `linux_dispatch` размаскирует IF только если она
  была размаскирована на входе (реальный syscall через `syscall_entry` — да, прямой вызов из
  boot-selftest'а — нет).
