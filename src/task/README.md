...
- **Загрузочный поток ≠ idle-задача.** До `halt_loop()` в `boot::kernel_main` текущий pid
  равен `IDLE_PID=0`, но стек принадлежит загрузочному потоку. Тик в этом окне трактует его
  как idle (`save_idle_rsp`) и уходит на всегда готовые shell/net-потоки — а загрузочный поток
  после этого не восстанавливается, потому что ready-queue никогда не пустеет. Поэтому тик
  **не должен** случаться до конца загрузки: `linux_dispatch` размаскирует IF только когда
  вызывающий передал ненулевой `reentry_allowed` — это делают оба входных стаба
  (`syscall_entry` через SFMASK и `int 0x80` через interrupt gate входят с замаскированной IF,
  поэтому решение принимается по ВЫЗЫВАЮЩЕМУ, а не по текущей IF: проверка текущей IF всегда
  видела бы ноль и не размаскировала бы ничего). Прямой вызов из boot-selftest'а передаёт 0 и
  оставляет IF как была.
