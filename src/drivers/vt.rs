...
impl Vt {
    fn new(cols: usize, rows: usize) -> Self {
        Self {
            cols,
            rows,
            cells: alloc::vec![Cell::BLANK; cols*rows],
            cx: 0,
            cy: 0,
            saved_cx: 0,
            saved_cy: 0,
    ...
    fn put(&mut self, x: usize, y: usize, c: Cell) {
        ...
        self.cells[i] = c;
        let px = x * GLYPH_W;
        let py = STATUS_H + y * GLYPH_H;
        crate::drivers::framebuffer::with(|fb| {
            fb.draw_glyph_px(c.ch, px, py, c.fg, c.bg);
        });
    }
    fn paint_all(&self) {
    ...
    fn p(&self, i: usize, def: u32) -> u32 {
        if i < self.np && self.params[i] != 0 {
            self.params[i]
        } else {
            def
        }
    ...
    fn erase_disp(&mut self, m: u32) {
        ...
        for i in s..e.min(self.cells.len()) {
            self.cells[i] = Cell::BLANK;
        }
        self.paint_all();
    }
    fn erase_line(&mut self, m: u32) {
        ...
        for col in s..e {
            self.put(col, self.cy, Cell::BLANK);
        }
    }
    fn newline(&mut self) {
    ...
    fn sgr(&mut self) {
        ...
        while i < self.np.max(1) {
            match self.params.get(i).copied().unwrap_or(0) {
                ...
                30..=37 => {
                    self.fg = ansi256(self.params[i] - 30);
                }
                38 => match self.params.get(i + 1).copied() {
                    Some(5) if i + 2 < self.np => {
                        self.fg = ansi256(self.params[i + 2]);
                        i += 2;
                    }
                    Some(2) if i + 4 < self.np => {
                        let (r, g, b) =
                            (self.params[i + 2], self.params[i + 3], self.params[i + 4]);
                        self.fg = (r << 16) | (g << 8) | b;
                        i += 4;
                    }
                    _ => {}
                ...
                40..=47 => {
                    self.bg = ansi256(self.params[i] - 40);
                }
                48 => match self.params.get(i + 1).copied() {
                    Some(5) if i + 2 < self.np => {
                        self.bg = ansi256(self.params[i + 2]);
                        i += 2;
                    }
                    Some(2) if i + 4 < self.np => {
                        let (r, g, b) =
                ...
                90..=97 => {
                    self.fg = ansi256(self.params[i] - 90 + 8);
                }
                100..=107 => {
                    self.bg = ansi256(self.params[i] - 100 + 8);
                }
                _ => {}
            }
            i += 1;
    ...
    fn csi(&mut self, f: u8) {
        match f {
            ...
            b'G' => {
                self.cx = (self.p(0, 1) as usize).saturating_sub(1).min(self.cols - 1);
            }
            b'H' | b'f' => {
            ...
            b'J' => {
                self.erase_disp(self.p(0, 0));
            }
            b'K' => {
                self.erase_line(self.p(0, 0));
            }
            b'L' => {
                let n = self.p(0, 1) as usize;
                self.scroll_dn(n);
            ...
            b'P' => {
                ...
                for col in self.cx..self.cols {
                    let c = if col + n < self.cols {
                        self.get(col + n, self.cy)
                    } else {
                        Cell::BLANK
                    };
                    let i = self.idx(col, self.cy);
                    self.cells[i] = c;
                }
                for col in self.cx..self.cols {
                    self.blit(col, self.cy);
                }
            }
            b'S' => {
                let n = self.p(0, 1) as usize;
            ...
            b'X' => {
                let n = (self.p(0, 1) as usize).min(self.cols - self.cx);
                for col in self.cx..self.cx + n {
                    self.put(col, self.cy, Cell::BLANK);
                }
            }
            b'd' => {
                self.cy = (self.p(0, 1) as usize).saturating_sub(1).min(self.rows - 1);
            }
            b'm' => {
                self.sgr();
            }
            b'r' => {
                ...
                let b = (self.p(1, self.rows as u32) as usize)
                    .saturating_sub(1)
                    .min(self.rows - 1);
                if t < b {
                    self.scroll_top = t;
    ...
    fn csi_final(&mut self, f: u8) {
        match (self.inter, self.inter2, f) {
            ...
            (b'>', 0, b'q') => {
                respond(b"\x1bP>|PaghVT 16.3\x1b\\");
            } // XTVERSION
    ...
    fn process_osc(&mut self) {
        ...
        if buf.starts_with(b"10;?") {
            respond(b"\x1b]10;rgb:aaaa/aaaa/aaaa\x1b\\");
        } else if buf.starts_with(b"11;?") {
            respond(b"\x1b]11;rgb:0000/0000/0000\x1b\\");
        }
    }
    fn blit(&self, x: usize, y: usize) {
        let c = self.get(x, y);
        let px = x * GLYPH_W;
        let py = STATUS_H + y * GLYPH_H;
        crate::drivers::framebuffer::with(|fb| {
    ...
    pub fn feed(&mut self, b: u8) {
        match self.state {
            Es::Normal => match b {
                ...
                0x09 => {
                    let n = (self.cx + 8) & !7;
                    self.cx = n.min(self.cols - 1);
                }
                0x0A | 0x0B | 0x0C => {
                    self.cx = 0;
                    self.newline();
                } // STAGE 16.15: LF implies CR (emulated ONLCR)
                0x0D => {
                    self.cx = 0;
                }
                0x1B => {
            ...
            Es::Csi => match b {
                b'0'..=b'9' => {
                    ...
                    self.params[i.min(15)] = self.params[i.min(15)]
                        .saturating_mul(10)
            ...
            Es::Osc => {
                if b == 0x07 {
                    ...
                    self.state = Es::OscEsc;
                } else if self.osc_buf.len() < 64 {
                    self.osc_buf.push(b);
                }
            }
            Es::OscEsc => {
            ...
            Es::Dcs => {
                if b == 0x07 {
                    ...
                    self.state = Es::DcsEsc;
                }
            }
            Es::Chs => {
                self.state = Es::Normal;
            } // charset designator byte: consume
            Es::DcsEsc => {
...
pub fn has_input_responses() -> bool {
    !RESPONSES.lock().is_empty()
}

/// Drain queued query replies for injection into the stdin stream.
pub fn take_input_responses() -> Vec<u8> {
    core::mem::take(&mut *RESPONSES.lock())
}
...
pub fn init() {
    ...
    let cols = fbw / GLYPH_W;
    let rows = (fbh.saturating_sub(STATUS_H)) / GLYPH_H;
    if cols == 0 || rows == 0 {
        return;
    }
    crate::drivers::framebuffer::with(|fb| {
        fb.fill_rect(0, STATUS_H, fbw, fbh - STATUS_H, DEFAULT_BG);
    });
...
pub fn dimensions() -> (u16, u16) {
    VT.lock()
        .as_ref()
        .map(|v| (v.cols as u16, v.rows as u16))
        .unwrap_or((80, 25))
}
