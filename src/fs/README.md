...
- On-disk транзакция = `[Descriptor][Data]*N[Commit]`; Descriptor держит до 254 таргетов,
  Commit — magic + seq + CRC32 по всем data-блокам. Коммит-запись — точка **атомарности**.
  `commit` шлёт `BlockDevice::flush()` дважды: после коммит-записи (журнал на носителе) и после
  checkpoint'а, до сдвига `head` (issue #15). На RAM-моке и устройствах без кэша `flush` — no-op.
