...
2. **Host property tests** (`host-tests/`) — `proptest` for logic that extracts cleanly to
   the host (currently **41** modules, `p01`–`p41`, covering the kernel core plus the
...
3. **Boot-time Linux/apt self-tests** (`src/selftest_lx.rs`, cargo feature `lx_selftest`) —
   ...
   the ELF loader (static and glibc-dynamic), and the `apt` pipeline against a local mirror; they print
