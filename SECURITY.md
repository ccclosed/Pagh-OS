...
**`AT_RANDOM` is best-effort when hardware entropy is missing (issue #16).** glibc reads the ELF `AT_RANDOM` block unconditionally at process start, so — unlike `getrandom`, which returns `EAGAIN` — it cannot fail closed: refusing would mean refusing to start the process. It prefers RDSEED/RDRAND; when the CPU exposes neither, the block is derived from a mixed boot seed (SHA-256 over TSC timing jitter, CMOS RTC port-read races, kernel counters and address-space layout) plus a per-process counter, none of which is derivable from the observable inputs (tick clock, RTC wall clock, pid) the previous xorshift fallback was built from. This is **best-effort, not a CSPRNG**: on a fully deterministic replay the collected seed is reproducible and so are the canaries. The degradation is therefore announced once at boot (`entropy: stage=degraded cause=NoHardwareEntropy cpu=…`) instead of being silent, and booting QEMU with `-cpu max` (RDSEED/RDRAND present) stays the recommended configuration. Host properties P51 prove the mixer's avalanche and secret separation and reject the removed xorshift plus degenerate mixers as negative controls; the in-guest `selftest` samples 64 blocks and prints a digest, which is what makes a `-cpu qemu64` run comparable across boots.
